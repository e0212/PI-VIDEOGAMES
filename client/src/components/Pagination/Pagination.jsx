...
export const Pagination = ({totalGames, gamesPerPage, page, setPage}) => {
  ...
  return (
    <div className="pagination">
      <button className="butttonGet" onClick={Previus}>Prev.</button>
        {pageNumbers.map((number) => (
          <button className="paginationButton" onClick={onChange} value={number}>{number}</button>
        ))}
      <button className="butttonGet" onClick={Next}>Next</button>
    </div>
  );
};
